...
if ($Port -ne 23010) {
    throw "V1-09 scene builder requires dedicated port 23010."
}
...
$Builder = Join-Path $PSScriptRoot "build_v1_09_scene.py"
...
try {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "V1-09 Python scene builder failed with exit code $LASTEXITCODE."
        }
    } finally {
        Pop-Location
    }
